...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Kirim email OTP
    await sendEmail(email, "Verivikasi email Tokoku", `
      <h3>Halo ${name},</h3>
...
router.post("/resend-otp", async (req, res) => {
  ...
  try {
    ...
    // Kirim email OTP
    await sendEmail(user.email, "Verifikasi Email Tokoku", `<h1>${otpCode}</h1>`)
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified) return res.status(401).json({ message: "Akun belum diverifikasi, silakan cek email OTP!" })
